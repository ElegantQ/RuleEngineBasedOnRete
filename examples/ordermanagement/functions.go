...
func init() {
	...
	configuration.RegisterConditionEvaluator("truecondition", truecondition)
	configuration.RegisterActionFunction("ordereventAction", ordereventAction)

	//rule itemevent, handle a item event, update order in the itemeventAction
	configuration.RegisterConditionEvaluator("itemeventCondition", itemeventCondition)
	configuration.RegisterActionFunction("itemeventAction", itemeventAction)

	//rule order
	configuration.RegisterConditionEvaluator("orderCondition", orderCondition)
	configuration.RegisterActionFunction("orderAction", orderAction)

	//rule ordertimeoutevent, order timeout if within a specified interval, all order items dont arrive
	configuration.RegisterConditionEvaluator("ordertimeouteventCondition", ordertimeouteventCondition)
	configuration.RegisterActionFunction("ordertimeouteventAction", ordertimeouteventAction)

	//rule ordershippedevent
	configuration.RegisterConditionEvaluator("ordershippedCondition", ordershippedCondition)
	configuration.RegisterActionFunction("ordershippedAction", ordershippedAction)

	//rule ordercancelled, notification on order cancelled
	configuration.RegisterConditionEvaluator("ordercancelledCondition", ordercancelledCondition)
	configuration.RegisterActionFunction("ordercancelledAction", ordercancelledAction)

	//rule shippingtimeoutevent, shipping timeout if within a specified interval, order is not shipped
	configuration.RegisterConditionEvaluator("shippingtimeouteventCondition", shippingtimeouteventCondition)
	configuration.RegisterActionFunction("shippingtimeouteventAction", shippingtimeouteventAction)

	//rule orderinvoice, apply discounts based on the customer level on hte final invoice
	configuration.RegisterConditionEvaluator("orderinvoiceCondition", orderinvoiceCondition)
	configuration.RegisterActionFunction("orderinvoiceAction", orderinvoiceAction)
}
