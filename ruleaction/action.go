...
var manager *configuration.ResourceManager

//var resManager *configuration.ResManager
...
func (f *ActionFactory) Initialize(ctx action.InitContext) error {
	...
	manager = configuration.NewResourceManager()
	resource.RegisterLoader(configuration.RESTYPE_RULESESSION, manager)
